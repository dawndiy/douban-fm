...
executeCommand "click chroot -a armhf -f $CHROOT -s $SERIES run CGO_ENABLED=1 GOARCH=arm GOARM=7 PKG_CONFIG_LIBDIR=/usr/lib/arm-linux-gnueabihf/pkgconfig:/usr/lib/pkgconfig:/usr/share/pkgconfig GOROOT=$GOROOT GOPATH=$CURRENT_DIR CC=arm-linux-gnueabihf-gcc CXX=arm-linux-gnueabihf-g++ $GOROOT/bin/go build -ldflags '-extld=arm-linux-gnueabihf-g++' -o $APP_NAME/$APP_NAME"
echo "Done"
